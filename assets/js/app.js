/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `NETPLUS` namespace with
   ...
   - quizEngine.js adds the 500+ question bank, the 210-object PBQ database, and
...
(function () {
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("netplus." + k, fb); },
      set: function (k, v) { write("netplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("netplus." + k); } catch (e) {} }
    };
  })();
  ...
  function statCard() {
    ...
    return el("div.kpis", null, [
      ...
      kpi(pbqDone + "/" + (S.pbqs ? S.pbqs.length : 210), "PBQs completed", ""),
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the N10-009 is scored and delivered, and where the credential fits in an IT/cybersecurity career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the N10-009 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where Network+ fits on the IT and networking career ladder, the infrastructure milestones it aligns with, and the junior network admin, NOC technician, and support roles it unlocks.",
      foot: "Foundational networking credential", cta: "Read",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every Network+ acronym across all five domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    var b6 = dashSection("06", "Performance-based question simulators", (S.pbqs ? S.pbqs.length : 210) + " simulations · " + (nFormats + 1) + " modules");
    b6.appendChild(el("p.sectsub", { html: "A database of <b>" + (S.pbqs ? S.pbqs.length : 210) + "</b> hands-on PBQs across " + nFormats + " formats. Read the exhibit, configure each field, and submit for graded feedback mapped to the objective." }));
    var g4 = el("div.grid");
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the N10-009 &mdash; 90 questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: Professor Messer's full N10-009 course, the official CompTIA objectives PDF, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.NETPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function computeReadiness() {
    ...
    var pbqTotal = (S.pbqs || []).length || 210, pbqDone = (Store.get("pbqDone", []) || []).length;
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates NETPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.NETPLUS_VER ? "?v=" + window.NETPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "N10-009 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: N10-009", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where Network+ fits in your path", S.careerGuidance || [])); }
